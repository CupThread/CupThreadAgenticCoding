...
| Endpoint | Method | Purpose |
|---|---|---|
| `/api/v1/console/workspaces/:wsId/feature-requests/:frId/comments` | `GET` | List every comment on a feature request (including hidden, excluding soft-deleted) for moderation. Keyset-paginated (PROD-31) like the public thread: `limit` 1–200 (default 200), opaque `cursor`, returns `{comments, total, hasMore, nextCursor}` where `total` counts non-deleted comments (hidden included) and `nextCursor` is `null` on the last page; `400 {"error": "Invalid cursor"}` for a malformed cursor. Returns `404` when the feature request is not in the workspace. |
