...
- **CLI / OAuth Access**: `cupthread auth login` runs Authorization Code + PKCE in a browser (or the RFC 8628 device flow on headless machines) against the OAuth authorization server and stores a rotating 14-day token pair. The whole surface is documented in the OpenAPI spec (tag `OAuth`) — see [OAuth Authorization Server (QUAL-04)](#oauth-authorization-server-qual-04).

---

## OAuth Authorization Server (QUAL-04)

The OAuth 2.0 authorization-server surface the CLI consumes is now fully documented in the OpenAPI 3.1 spec (`https://api.cupthread.com/api/v1/openapi.json`, browsable at `https://api.cupthread.com/reference`). For automated discovery, **`GET /.well-known/oauth-authorization-server`** serves RFC 8414 metadata: `issuer`, `authorization_endpoint`, `token_endpoint`, `revocation_endpoint`, `device_authorization_endpoint`, the supported response/grant types, `code_challenge_methods_supported: ["S256"]`, token-endpoint auth methods, and `scopes_supported: ["full"]`.

| Endpoint | Contract |
|---|---|
| `GET /api/v1/oauth/authorize` | Authorization Code + PKCE start (RFC 6749 §4.1.1 / RFC 7636): validates the request, then **302-redirects to the Console consent page** carrying the OAuth parameters. Validation failures answer RFC 6749 §5.2 JSON errors (`invalid_request`, `unsupported_response_type`, `invalid_client`, `invalid_scope`) **instead of a redirect** — an unregistered `redirect_uri` must never become an open redirect. Only `response_type=code`, `code_challenge_method=S256`, and scope `full` are supported (`code_challenge` must match `^[A-Za-z0-9\-_]{43,128}$`). Redirect URIs must be absolute `https:` URLs or loopback `http://127.0.0.1` / `http://[::1]` / `http://localhost` (any port, RFC 8252 §7.3); custom schemes and non-loopback `http:` are rejected (SEC-46). |
| `POST /api/v1/oauth/authorize` | The signed-in user's consent decision (interactive Clerk session required; a `cpt_` token is rejected with `403`). `allow` mints a single-use 10-minute authorization code bound to client, redirect URI, PKCE challenge, and user, and answers `{ redirectTo }` with `?code=…&state=…`; `deny` answers `{ redirectTo }` with `?error=access_denied&state=…`. |
| `POST /api/v1/oauth/token` | The two browser-flow grants: `authorization_code` (send `code`, the matching `redirect_uri`, and the RFC 7636 §4.6 `code_verifier`) and `refresh_token` (rotates the pair; see below). Form-urlencoded per RFC 6749 §2.3.1; a JSON object with string values is accepted as a compatibility encoding; `multipart/form-data` is rejected with `415`. Confidential clients (registered with a secret) must send `client_secret` — a missing or wrong one is `401`. Rate-limited per client id + client IP (`429`). |
| `POST /api/v1/oauth/device/authorize` | RFC 8628 §3.1 device start: returns `device_code`, `user_code`, `verification_uri`, `verification_uri_complete`, `expires_in` = 900 (15 minutes), and `interval` = 5 (minimum seconds between token polls). |
| `POST /api/v1/oauth/device/token` | RFC 8628 §3.4/§3.5 polling with `grant_type=urn:ietf:params:oauth:grant-type:device_code`. While the user decides: `400 authorization_pending` (keep polling) and `400 slow_down` (grow the interval by 5 seconds). Terminal answers: `400 access_denied` (user denied), `400 expired_token` (restart the flow), and `invalid_grant` when an already-redeemed code is polled again — approval issues the pair exactly once. |
| `POST /api/v1/oauth/revoke` | RFC 7009 revocation: form (or JSON) body `token` (+ optional `client_id`); revokes the presented access or refresh token **and its sibling — the whole pair dies together**. Per RFC 7009 §2.2 the endpoint **always returns `200` with an empty body**, even for unknown or already-revoked tokens, so token existence is never disclosed. |
| `GET /api/v1/oauth/consent-info`, `GET /api/v1/oauth/device/lookup`, `POST /api/v1/oauth/device/decide` | Session-scoped Console helpers backing the consent and device-confirmation pages (`{ error }` bodies, not RFC 6749 §5.2) — not part of the CLI's token flows. |

All three grants answer the same envelope: `{ access_token, refresh_token, token_type: "Bearer", expires_in, scope }`. Access tokens are `cpt_…` bearer tokens living **14 days** (`expires_in` = 1209600); refresh tokens live **180 days** and **rotate on every use** — the old refresh token is revoked atomically, so replaying a rotated token fails with `invalid_grant`. Scope is always `full`. Every grant-endpoint error is the RFC 6749 §5.2 shape `{ error, error_description }` (`error_description` is advisory text, not a stable contract).
...
| `/api/v1/public/columns/:appKey` | `GET` | Roadmap Kanban columns sorted by position. Apps with anonymous roadmap view disabled answer `401` `authentication_required` (PRIV-12) instead of the column list — see [Per-App Anonymous-Access Enforcement (PRIV-12)](#per-app-anonymous-access-enforcement-priv-12). |
| `/api/v1/public/versions/:appKey` | `GET` | Release versions sorted by position. Only rows whose `released` flag is true are listed (PROD-32) — unreleased versions stay console-private until a developer marks them released, and a request bound to an unreleased version still shows on the board with its version simply absent from this list. Same `401` `authentication_required` (PRIV-12) on sign-in-only roadmaps as the columns route. |
...
| `/api/v1/public/apps/:appKey/changelog/subscribe` | `POST` | Subscribe an email to changelog updates (double opt-in). Success is always `201 {"subscribed": true}` — the former `alreadySubscribed` field was removed, so the response never reveals prior state. The address starts *pending* and a single-use confirmation email is sent (resends within a 15-minute cooldown return `201` without dispatching a duplicate email). Changelogs with anonymous access disabled reject anonymous calls with `401` `authentication_required` and bind signed-in calls to the session's verified address: a body `email` that is not one of them fails with `403 {"error": …, "code": "email_not_verified"}` (PRIV-12). Rate limited per client IP (`429`). |
...
| `/api/v1/feature-requests/:id/vote` | `DELETE` | Explicitly remove the caller's vote, returns `{voted: false, voteCount}`. Same per-client-IP vote rate limit applies (`429`). Body-size note (SEC-36): the JSON body is read through a 256 KB bounded reader **before** the vote rate limit, so an oversized body fails with `413 payload_too_large` instead of `429`. |
| `/api/v1/feature-requests/:id/comments` | `GET` | List comments and @replies on a feature request. Keyset-paginated (PROD-31): pages walk `(createdAt, id)` oldest-first with `limit` 1–200 (default 200) and an opaque `cursor` from the previous page's `nextCursor`; returns `{comments, total, hasMore, nextCursor}` where `total` is the thread size under public visibility rules (hidden and deleted excluded — not `comments.length`) and `nextCursor` is `null` on the last page. Clients assuming one fetch returns the whole thread must walk `nextCursor`: the old silent 500-row cap is gone and every request returns at most 200 rows. A malformed cursor fails with `400 {"error": "Invalid cursor"}`. | Sign-in-only boards (anonymous roadmap view disabled) answer `401` `authentication_required` to anonymous reads, and threads of **unapproved** requests return `404` — the same existence hiding as the board listing, so never infer "deleted" from a `404` (PRIV-12). |
...
| `/api/v1/uploads/images` | `POST` | Legacy multipart image upload — **now requires an upload session** (`401` `upload_session_required` without one; `Authorization` header or `sessionToken`/`sessionId` form field). PNG/JPEG/WebP/GIF only — SVG and declared-vs-content mismatches fail with `415` (see the media-type policy below). Metered (SEC-21): the workspace **daily storage quota** — bytes actually stored, not declared — and the submit rate limit both answer `429` (`daily_storage_quota_exceeded` for the quota). The stored image's `key`/`url` is publicly served via `GET /api/v1/files/:key`. |
| `/api/v1/feedback/attachments/:id/download` | `GET` | Private attachment download (authorized). Signed `?token=` link or workspace-member auth; anonymous callers without a valid token get `403`. `GET …/attachments/:id` is an alias of this route. |
| `/api/v1/files/:key` | `GET` | Public image serving for `images/…` storage keys (the URL-encoded `key`/`url` returned by `POST /api/v1/uploads/images`). Served with the stored content type, `ETag`, `Cache-Control: public, max-age=60`, and SEC-13 hardening: `X-Content-Type-Options: nosniff` plus `Content-Security-Policy: sandbox`, and legacy pre-screening SVG objects are forced to download via `Content-Disposition` instead of executing. Private attachment keys never resolve here — `403 private_attachment_forbidden`; fetch those through the authorized `/api/v1/feedback/attachments/:id/download` endpoint instead. |
...
Public write endpoints — and, since SEC-34, the unauthenticated profile read — are budgeted **per client IP** (keyed on `CF-Connecting-IP`). Throttled requests get `429 {"error": "Too many requests. Please try again shortly."}` — except the feature-request vote endpoints, which return a vote-specific body: `429 {"error": "Too many votes. Please try again shortly."}`. Ordering caveat (SEC-36): request-body limits run **before** rate limiting on every intake route, so an oversized body is answered with `413` (see [Request Body Limits (SEC-36)](#request-body-limits-sec-36)) even where a `429` would otherwise come first:

| Endpoints | Budget | Why |
|---|---|---|
| `POST .../changelog/subscribe`, `GET`/`POST .../changelog/confirm`, `GET .../changelog/unsubscribe`, `GET .../public/digest/unsubscribe` | 10 requests / 60 s | Subscribe emails third parties and confirm/unsubscribe mutate subscription state, so the budget is tight. These endpoints **share one per-IP bucket** (`PUBLIC_WRITE_RATE_LIMITER`). |
| `POST .../changelog/unsubscribe` and `POST .../public/digest/unsubscribe` **with a token** | 300 requests / 60 s | RFC 8058 one-click unsubscribes (PRIV-08): mailbox providers POST from a handful of shared egress IPs, so token-bearing POSTs (query `?token=` or body/form token, whitespace-trimmed non-empty) get their own per-IP budget (`DIGEST_UNSUBSCRIBE_RATE_LIMITER`) — 30× the interactive bucket above. Token-less POSTs stay on the 10/min public-write bucket as brute-force defense. Same generic `429` body. |
...
## Request Body Limits (SEC-36)

Every intake route parses JSON, form, and multipart bodies through bounded readers **before** rate limiting, signature checks, or validation — a request over the limit is rejected with no rate-limit counter tick and no database work:

| Surface | Limit |
|---|---|
| Webhooks (GitHub / Polar / Clerk) and public token endpoints | 1 MB |
| Public JSON intake — feature requests, votes, comments, `PUT .../user`, `POST /feedback`, upload sessions, changelog subscribe/confirm/unsubscribe, digest unsubscribe | 256 KB |
| Console JSON (authenticated `cupthread` surfaces) | 1 MB |
| Console image multipart (app icon upload) | 11 MB (10 MB image + multipart envelope) |

- Over-limit → `413 {"error": "Payload exceeds size limit", "code": "payload_too_large"}`.
- Malformed `Content-Length` header (negative or non-integer) → `400 {"error": "Invalid Content-Length header", "code": "invalid_content_length"}`.

The ordering is wire-observable: `DELETE /api/v1/feature-requests/:id/vote` with an oversized body returns `413` **before** the vote rate limit can return `429`. Client guidance: keep end-user-generated JSON bodies well under the 256 KB public budget — truncate or drop oversized free-text fields before submitting rather than discovering the limit via `413`. The CLI shares the same surfaces: `cupthread apps update --icon` pre-checks the 10 MB image cap client-side (inside the server's 11 MB multipart envelope), and `cupthread api request` bodies over 1 MB on console routes (256 KB on public routes) get the `413` body passed through.
...
The per-app `PublicAppConfig` anonymous-access flags (`allowAnonymousRoadmap`, `allowAnonymousChangelog`) are enforced **server-side on every surface of the family**, not just the primary listing GET. When a tenant disables a flag, the other previously-public reads and writes of that family go behind sign-in immediately; response schemas are unchanged — only new error conditions appear (source: SaaS PR #330, issue #324).

| Endpoint | Condition | Response |
|---|---|---|
| `GET /api/v1/feature-requests` | anonymous read, `allowAnonymousRoadmap` off | `401` `authentication_required` (pre-existing) |
| `GET /api/v1/public/columns/:appKey` | anonymous read, `allowAnonymousRoadmap` off | `401` `authentication_required` |
| `GET /api/v1/public/versions/:appKey` | anonymous read, `allowAnonymousRoadmap` off | `401` `authentication_required` |
| `GET /api/v1/feature-requests/:id/comments` | anonymous read, `allowAnonymousRoadmap` off | `401` `authentication_required` |
| `GET /api/v1/feature-requests/:id/comments` | request exists but is **unapproved** | `404` `{"error": "Feature request not found"}` — existence hiding, indistinguishable from a missing id |
| `POST /api/v1/public/apps/:appKey/changelog/subscribe` | anonymous call, `allowAnonymousChangelog` off | `401` `authentication_required` |
| `POST /api/v1/public/apps/:appKey/changelog/subscribe` | signed-in, body `email` is not the session's verified address | `403` `{"error": "Subscriptions on this changelog are bound to your signed-in email address", "code": "email_not_verified"}` |

The `401` body is uniform across the roadmap surfaces: `{"error": "Sign in is required to view this roadmap" | "Sign in is required to view this changelog", "code": "authentication_required"}`. Note that the comments-thread `401` check runs **after** existence hiding, so while the flag is off anonymous probers cannot distinguish existing ids from missing ones either.
...
Both verbs are rate limited per client IP (`429 {"error": "Too many requests. Please try again shortly."}`). The GET stays on the shared 10 requests / 60 s public-write budget; since PRIV-08 a POST carrying a token (query or body/form, whitespace-trimmed non-empty) draws from the dedicated `DIGEST_UNSUBSCRIBE_RATE_LIMITER` at 300 requests / 60 s — sized for mailbox providers one-click-unsubscribing from shared egress IPs — while token-less POSTs stay on the 10/min public-write bucket.
