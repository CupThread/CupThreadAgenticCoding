...
| `/api/v1/feature-requests/:id/comments` | `GET` | List comments and @replies on a feature request. Sign-in-only boards (anonymous roadmap view disabled) answer `401` `authentication_required` to anonymous reads, and threads of **unapproved** requests return `404` — the same existence hiding as the board listing, so never infer "deleted" from a `404` (PRIV-12). |
