...
Hermes does not expose a global `crypto.subtle` — polyfill with `react-native-quick-crypto` (or use `expo-crypto`'s `HmacDigest`) before calling this on device.
